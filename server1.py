...
server1_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # ✅ Allow immediate port reuse
server1_socket.bind((HOST, PORT))
server1_socket.listen(1)

print("Server 1 is waiting for a connection...")
...
try:
    ...
    print(f"Server 1 received ID: {received_id}")
    ...
    Enc_p1 = ts.bfv_vector_from(context, receive_large_data(conn))
    t_1 = np.load(f"server1_database/{received_id}/{received_id}d0.npy")
    t_1 = t_1*1000
    Enc_t1 = ts.bfv_vector(context, t_1.tolist())

    # 6️⃣ Compute difference
    Enc_diff = Enc_t1 - Enc_p1
